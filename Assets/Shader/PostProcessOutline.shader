    ...
    SubShader
    {
        ...
        Pass
        {
		    ...
		    float4 Frag (v2f i) : SV_Target
            {
				...
				float2 uv0 = i.uv + float2(-offset.x, 0).x;
				float2 uv1 = i.uv + float2(offset.x, 0).x;
				float2 uv2 = i.uv + float2(0, offset.y).x;
				float2 uv3 = i.uv + float2(0, -offset.y).x;

				float left = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uv0));
				float right = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uv1));
				float up = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uv2));
				float down = Linear01Depth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, uv3));

				float delta = sqrt(pow(right - left, 2) + pow(up - down, 2));
				float t = smoothstep(_MinDepth, _MaxDepth, delta);

				float4 color = lerp(original, _EdgeColor, _EdgeColor.a);

				float4 output = lerp(original, color, t);
                return output;
            }
